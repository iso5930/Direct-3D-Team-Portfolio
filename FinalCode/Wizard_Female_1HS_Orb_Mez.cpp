...
CWizard_Female_1HS_Orb_Mez::CWizard_Female_1HS_Orb_Mez(int _iMez , D3DXVECTOR3& _vPos)
{
	m_eStateType = STATE_TYPE_WIZARD_FEMALE_1HS_ORB_MEZ;
...
CWizard_Female_1HS_Orb_Mez::~CWizard_Female_1HS_Orb_Mez(void)
{
}

void CWizard_Female_1HS_Orb_Mez::Initialize()
{
...
CPlayerState* CWizard_Female_1HS_Orb_Mez::Action()
{
	switch (m_iMez)
	{
...
CPlayerState* CWizard_Female_1HS_Orb_Mez::Mode0()
{
...
CPlayerState* CWizard_Female_1HS_Orb_Mez::Mode1()
{
	return new CWizard_Female_1HS_Orb_Knockback(m_vPos, 1000.f);
}

CPlayerState* CWizard_Female_1HS_Orb_Mez::Mode2()
{
	return new CWizard_Female_1HS_Orb_Knockback(m_vPos, 1000.f);
}

CPlayerState* CWizard_Female_1HS_Orb_Mez::Mode3()
{
...
CPlayerState* CWizard_Female_1HS_Orb_Mez::SlotAction(int _iIndex)
{
	...
	case 	SLOT_ID_WIZARD_MAGIC_MISSILE:					// 마력탄
		if (CItemMgr::GetInstance()->GetItemInEquip(9) != NULL && CItemMgr::GetInstance()->GetItemInEquip(9)->dwItemID & WEAPON_ID_WAND)
		{
			...
			iRand = rand() % 2;
			if (iRand ==0 )
				return new CWizard_Female_Wand_Attack;
			else
				return  new CWizard_Female_1HS_Attack; 
		}
		else
			return  new CWizard_Female_1HS_Attack; 

	case	SLOT_ID_WIZARD_SHOCK_PULSE:						// 전기 충격
		return new CWizard_Female_1HS_Directed;  

	case	SLOT_ID_WIZARD_SPECTRAL_BLADE:					// 저승의 칼날
		return new CWizard_Female_1HS_AOE;

	case	SLOT_ID_WIZARD_ELECTROCUTE:						// 감전
		return new CWizard_Female_1HS_Channel(_iIndex);
	...
	case	SLOT_ID_WIZARD_DIAMOND_SKIN:					// 다이아몬드 피부		
		return new CWizard_Female_1HS_Summon;				

	case	SLOT_ID_WIZARD_SLOW_TIME:						// 감속 지대			
		return new CWizard_Female_1HS_Summon;				

	case	SLOT_ID_WIZARD_TELEPORT:						// 순간이동	
		return new CWizard_Female_1HS_Teleport;				

	case	SLOT_ID_WIZARD_WAVE_OF_FORCE:					// 힘의 파동			
		return new CWizard_Female_1HS_AOE2;						
...
bool CWizard_Female_1HS_Orb_Mez::IsSlotRange(int _iIndex)
{
	//눌린 키 저장 
