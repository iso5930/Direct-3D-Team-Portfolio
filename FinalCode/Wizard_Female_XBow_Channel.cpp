...
CWizard_Female_XBow_Channel::CWizard_Female_XBow_Channel(int _iPushSlot)
{
	m_eStateType = STATE_TYPE_WIZARD_FEMALE_XBOW_CHANNEL;
...
CWizard_Female_XBow_Channel::~CWizard_Female_XBow_Channel(void)
{
...
void CWizard_Female_XBow_Channel::Initialize()
{
...
CPlayerState* CWizard_Female_XBow_Channel::Action()
{
...
CPlayerState* CWizard_Female_XBow_Channel::Mode0()
{
	...
	if (IsSlotRange(iSlotNum))
		...
		return new CWizard_Female_XBow_Run(m_pOwner->GetLastPushSlotNum());

};

CPlayerState* CWizard_Female_XBow_Channel::Mode1()
{
	// Find Priority Key and SlotNo
	int iSlotNum = CPlayerState::UsingKeyMouse();
...
CPlayerState* CWizard_Female_XBow_Channel::SlotAction(int _iIndex)
{
...
bool CWizard_Female_XBow_Channel::IsSlotRange(int _iIndex)
{
	//눌린 키 저장 
