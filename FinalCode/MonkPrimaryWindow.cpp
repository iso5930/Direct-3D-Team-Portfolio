...
CMonkPrimaryWindow::CMonkPrimaryWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_Monk_Primary"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("천둥주먹"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("연속 격파"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("마비의 파동"));
	_tcscpy_s(m_tszName[3], MIN_STR, _T("천지권법"));
	...
	_tcscpy_s(m_tszLevel[3], MIN_STR, _T("15"));
...
CMonkPrimaryWindow::~CMonkPrimaryWindow()
{
	Release();
}

void CMonkPrimaryWindow::Initialize()
{
	...
	// FistsOfThunderButton(6)
	CSlotButton* pFistsOfThunderButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_PRIMARY, SLOT_ID_MONK_FISTS_OF_THUNDER);
	m_pLayer->AddObject(pFistsOfThunderButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pFistsOfThunderButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pFistsOfThunderButton->Initialize();
	AddButton(pFistsOfThunderButton);

	// DeadlyReachButton(7)
	CSlotButton* pDeadlyReachButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_PRIMARY, SLOT_ID_MONK_DEADLY_REACH);
	m_pLayer->AddObject(pDeadlyReachButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pDeadlyReachButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pDeadlyReachButton->Initialize();
	AddButton(pDeadlyReachButton);

	// CripplingWaveButton(8)
	CSlotButton* pCripplingWaveButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_PRIMARY, SLOT_ID_MONK_CRIPPLING_WAVE);
	m_pLayer->AddObject(pCripplingWaveButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pCripplingWaveButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pCripplingWaveButton->Initialize();
	AddButton(pCripplingWaveButton);

	// WayOfTheHundredFistsButton(9)
	CSlotButton* pWayOfTheHundredFistsButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_PRIMARY, SLOT_ID_MONK_WAY_OF_THE_HUNDRED_FISTS);
	m_pLayer->AddObject(pWayOfTheHundredFistsButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pWayOfTheHundredFistsButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pWayOfTheHundredFistsButton->Initialize();
	AddButton(pWayOfTheHundredFistsButton);
...
int CMonkPrimaryWindow::Update()
{
	...
	if(m_ButtonVec[3]->IsLUp())
		CUIMgr::GetInstance()->NewWindow(WINDOW_TYPE_MONK_MANTRAS, m_iIndex, m_eSlotID);

	// RightButton
	if(m_ButtonVec[4]->IsLUp())
		CUIMgr::GetInstance()->NewWindow(WINDOW_TYPE_MONK_SECONDARY, m_iIndex, m_eSlotID);
...
void CMonkPrimaryWindow::Render()
{
	CWindowUI::Render();
...
void CMonkPrimaryWindow::Release()
{
	// DownArrowSign
