#include "StdAfx.h"
#include "Wizard_Female_1HS_Orb_Attack.h"

CWizard_Female_1HS_Orb_Attack::CWizard_Female_1HS_Orb_Attack(void)
{
	m_eStateType = STATE_TYPE_WIZARD_FEMALE_1HS_ORB_AOE2;
}

CWizard_Female_1HS_Orb_Attack::~CWizard_Female_1HS_Orb_Attack(void)
{
}

void CWizard_Female_1HS_Orb_Attack::Initialize()
{
	CPlayerState::Initialize();

	// SetAnimation
	m_pOwner->SetAnimation(46);

	// Clear
	m_pPathFindCom->Clear();

	if(m_pOwner->GetSlot(m_pOwner->GetLastPushSlotNum())->GetSlotID() == SLOT_ID_WIZARD_PUNCH ||
		m_pOwner->GetSlot(m_pOwner->GetLastPushSlotNum())->GetSlotID() == SLOT_ID_WIZARD_NEAR_ATTACK )
		m_pOwner->CreateAttack();
}

CPlayerState* CWizard_Female_1HS_Orb_Attack::Action()
{
	if(m_pAnimController->m_dFrameTime - 0.25 <= m_pAnimController->GetTrackPos())
	{
		m_pOwner->RemoveAttack();
		return new CWizard_Female_1HS_Orb_Idle;
	}
	return NULL;
}