...
CSkeleton_Archer_Mint_Idle::CSkeleton_Archer_Mint_Idle(int _iMode)
{
	// StateType
	m_eStateType = STATE_TYPE_SKELETON_ARCHER_MINT_IDLE;
...
CSkeleton_Archer_Mint_Idle::~CSkeleton_Archer_Mint_Idle(void)
{
}

void CSkeleton_Archer_Mint_Idle::Initialize()
{
	CMonsterState::Initialize();
	m_pOwner->SetAnimation(0);
}

CMonsterState* CSkeleton_Archer_Mint_Idle::Action()
{
	switch(m_iMode)
	{
	case 0:
...
CMonsterState* CSkeleton_Archer_Mint_Idle::Mode0()
{
	// Player
		...
		if(m_pPathFindCom->IsMove(&vNextPos))
		{
			...
			if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
			{
				...
				if(Equals(fDotZ, 1.0f, MAX_EPSILON))
				{
					...
					//공격
					return new CSkeleton_Archer_Mint_Run(0);
			...
			if(m_pPathFindCom->IsMove(&vNextPos))
			{
				...
				if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
				{
					...
					if(Equals(fDotZ, 1.0f, MAX_EPSILON))
					{
						...
						//Walk
						return new CSkeleton_Archer_Mint_Walk;
...
CMonsterState* CSkeleton_Archer_Mint_Idle::Mode1()
{
