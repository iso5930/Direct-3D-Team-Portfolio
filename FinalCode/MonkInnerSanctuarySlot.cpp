...
CMonkInnerSanctuarySlot::CMonkInnerSanctuarySlot()
{
	// SlotType
	m_eSlotType = SLOT_TYPE_MONK_DEFENSIVE;

	// SlotID
	m_eSlotID = SLOT_ID_MONK_INNER_SANCTUARY;
...
SLOT_RESULT CMonkInnerSanctuarySlot::Begin(int _iMode)
{
	// Check Enable
	if (!m_bEnable)
	{
	...
	CMonk_InnerSanctuary_DecalEffect* pDecalEffect = new CMonk_InnerSanctuary_DecalEffect(NULL, OBJ_TYPE_DYNAMIC, m_pOwner);
...
SLOT_RESULT CMonkInnerSanctuarySlot::Action()
{
	return SLOT_RESULT_NULL;
}

SLOT_RESULT CMonkInnerSanctuarySlot::End()
{
	return SLOT_RESULT_NULL;
}
