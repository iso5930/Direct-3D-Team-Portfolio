...
CSkeleton_Summoner_Red_Summon::CSkeleton_Summoner_Red_Summon(void)
{
	// StateType
	m_eStateType = STATE_TYPE_SKELETON_SUMMONER_RED_SUMMON;
...
void CSkeleton_Summoner_Red_Summon::Initialize()
{
...
CMonsterState* CSkeleton_Summoner_Red_Summon::Action()
{
	// Player
	...
	if(!m_bEffect && m_pAnimController->GetTrackPos() > 2.343f)
	{
		...
		D3DXVec3TransformCoord(&pTransformCom->m_vPos, &pTransformCom->m_vPos, &((CSkeleton_Summoner_Red*)m_pOwner)->m_RightHandBone.mWorld);
