...
CMonkTechniquesWindow::CMonkTechniquesWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_Monk_Techniques"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("Áø°ÝÅ¸"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("Æø¹ß Àå¹ý"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("ÈÖ¸ôÀÌ"));
...
CMonkTechniquesWindow::~CMonkTechniquesWindow()
{
	Release();
}

void CMonkTechniquesWindow::Initialize()
{
	...
	// DashingStrikeButton(6)
	CSlotButton* pDashingStrikeButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_TECHNIQUES, SLOT_ID_MONK_DASHING_STRIKE);
	m_pLayer->AddObject(pDashingStrikeButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pDashingStrikeButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pDashingStrikeButton->Initialize();
	AddButton(pDashingStrikeButton);

	// ExplodingPalmButton(7)
	CSlotButton* pExplodingPalmButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_TECHNIQUES, SLOT_ID_MONK_EXPLODING_PALM);
	m_pLayer->AddObject(pExplodingPalmButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pExplodingPalmButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.0f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pExplodingPalmButton->Initialize();
	AddButton(pExplodingPalmButton);

	// SweepingWindButton(8)
	CSlotButton* pSweepingWindButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_TECHNIQUES, SLOT_ID_MONK_SWEEPING_WIND);
	m_pLayer->AddObject(pSweepingWindButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pSweepingWindButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pSweepingWindButton->Initialize();
	AddButton(pSweepingWindButton);
...
int CMonkTechniquesWindow::Update()
{
	CWindowUI::Update();
	...
	if(m_ButtonVec[3]->IsLUp())
		CUIMgr::GetInstance()->NewWindow(WINDOW_TYPE_MONK_DEFENSIVE, m_iIndex, m_eSlotID);
...
void CMonkTechniquesWindow::Render()
{
	CWindowUI::Render();
...
void CMonkTechniquesWindow::Release()
{
	// DownArrowSign
...
void CMonkTechniquesWindow::Renew()
{
	// Select
	for(int i = 6; i < 9; ++i)
	{
