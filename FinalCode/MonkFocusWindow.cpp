...
CMonkFocusWindow::CMonkFocusWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_Monk_Focus"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("용오름"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("칠면 공격"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("신비한 벗"));
...
CMonkFocusWindow::~CMonkFocusWindow()
{
	Release();
}

void CMonkFocusWindow::Initialize()
{
	...
	// CycloneStrikeButton(6)
	CSlotButton* pCycloneStrikeButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_FOCUS, SLOT_ID_MONK_CYCLONE_STRIKE);
	m_pLayer->AddObject(pCycloneStrikeButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pCycloneStrikeButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pCycloneStrikeButton->Initialize();
	AddButton(pCycloneStrikeButton);

	// SevenSidedStrikeButton(7)
	CSlotButton* pSevenSidedStrikeButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_FOCUS, SLOT_ID_MONK_SEVEN_SIDED_STRIKE);
	m_pLayer->AddObject(pSevenSidedStrikeButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pSevenSidedStrikeButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.0f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pSevenSidedStrikeButton->Initialize();
	AddButton(pSevenSidedStrikeButton);

	// MysticAllyButton(8)
	CSlotButton* pMysticAllyButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_FOCUS, SLOT_ID_MONK_MYSTIC_ALLY);
	m_pLayer->AddObject(pMysticAllyButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pMysticAllyButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pMysticAllyButton->Initialize();
	AddButton(pMysticAllyButton);
