...
CDemonHunter_Female_BigBow_Idle::CDemonHunter_Female_BigBow_Idle()
{
	// StateType
	m_eStateType = STATE_TYPE_DEMONHUNTER_FEMALE_BIGBOW_IDLE;
...
void CDemonHunter_Female_BigBow_Idle::Initialize()
{
...
CPlayerState* CDemonHunter_Female_BigBow_Idle::Action()
{
	for (int i = 0 ; i < SKILL_TYPE_END ; ++i)
	{
		if (m_pOwner->IsMez(i))
		{
			return new CDemonHunter_Female_BigBow_Mez(i , D3DXVECTOR3(0.f , 0.f , 0.f ));
		}
	}
...
CPlayerState* CDemonHunter_Female_BigBow_Idle::Mode0()
{
...
CPlayerState* CDemonHunter_Female_BigBow_Idle::Mode1()
{
	...
	if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
	{
		...
		if(Equals(fDotZ, 1.0f, MAX_EPSILON))
		{
			...
			return new CDemonHunter_Female_BigBow_Run(m_pOwner->GetLastPushSlotNum());
		}
		else
		{
			if(fDotX > 0.0f)
...
CPlayerState* CDemonHunter_Female_BigBow_Idle::SlotAction(int _iIndex)
{
...
bool CDemonHunter_Female_BigBow_Idle::IsSlotRange(int _iIndex)
{
	//눌린 키 저장 
