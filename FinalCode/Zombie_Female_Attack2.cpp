...
CZombie_Female_Attack2::CZombie_Female_Attack2()
{
	// StateType
	m_eStateType = STATE_TYPE_ZOMBIE_FEMALE_ATTACK2;
...
void CZombie_Female_Attack2::Initialize()
{
	...
	CSoundMgr::GetInstance()->PlaySoundForMonster(_T("Zombie_Female_Attack2.ogg"));
}

CMonsterState* CZombie_Female_Attack2::Action()
{
	m_fEffectCreateTime += CTimeMgr::GetInstance()->GetDeltaTime();

	if(m_fEffectCreateTime > 0.363f && !m_bEffect)
	{
		CZombie_Female_Scratch_B* pScratchB = new CZombie_Female_Scratch_B(NULL, OBJ_TYPE_DYNAMIC, m_pOwner, &(m_pTransformCom->m_vPos));
...
CMonsterState* CZombie_Female_Attack2::Mode0()
{
...
CMonsterState* CZombie_Female_Attack2::Mode1()
{
	m_fTime += CTimeMgr::GetInstance()->GetDeltaTime();
	if(m_fTime > 1.f)
	{
