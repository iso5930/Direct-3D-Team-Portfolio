...
CBarbarian_Male_1HS_Mez::CBarbarian_Male_1HS_Mez(int _iMez , D3DXVECTOR3& _vPos)
{
	m_eStateType = STATE_TYPE_BARBARIAN_MALE_1HS_MEZ;
...
CBarbarian_Male_1HS_Mez::~CBarbarian_Male_1HS_Mez(void)
{
}

void CBarbarian_Male_1HS_Mez::Initialize()
{
...
CPlayerState* CBarbarian_Male_1HS_Mez::Action()
{
	switch (m_iMez)
	{
...
CPlayerState* CBarbarian_Male_1HS_Mez::Mode0()
{
...
CPlayerState* CBarbarian_Male_1HS_Mez::Mode1()
{
	return new CBarbarian_Male_1HS_Knockback_Loop(m_vPos, 1000.f);
}

CPlayerState* CBarbarian_Male_1HS_Mez::Mode2()
{
	return new CBarbarian_Male_1HS_Knockback_Loop(m_vPos, 1000.f);
}

CPlayerState* CBarbarian_Male_1HS_Mez::Mode3()
{
...
CPlayerState* CBarbarian_Male_1HS_Mez::SlotAction(int _iIndex)
{
	...
	case SLOT_ID_BARBARIAN_BASH:					// 후려치기
		{
			int iRand = rand() % 2; 
			if( 0 == iRand )
				return new CBarbarian_Male_STF_Attack;
			else
				return new CBarbarian_Male_STF_Cleave02;
		}
	case SLOT_ID_BARBARIAN_CLEAVE:					// 가르기
		{
			int iRand = rand() % 2; 
			if( 0 == iRand )
				return new CBarbarian_Male_STF_Cleave01;
			else
				return new CBarbarian_Male_STF_Cleave02;
		}
	...
	case SLOT_ID_BARBARIAN_HAMMER_OF_THE_ANCIENTS:	// 선조의 망치
		return new CBarbarian_Male_STF_Revenge;
	...
	case SLOT_ID_BARBARIAN_WHIRLWIND:				// 소용돌이
		return new CBarbarian_Male_STF_HirlWind(_iIndex);

	case SLOT_ID_BARBARIAN_ANCIENT_SPEAR:			// 고대의 작살
		return new CBarbarian_Male_STF_Attack;

	case SLOT_ID_BARBARIAN_GROUND_STOMP:			// 발 구르기
		return  new CBarbarian_Male_STF_FootStomp;

	case SLOT_ID_BARBARIAN_LEAP:					// 도약 공격
		return  new CBarbarian_Male_STF_LeapAttack_Air;

	case SLOT_ID_BARBARIAN_SPRINT:					// 질주
		return new CBarbarian_Male_STF_Buff;
	...
	case SLOT_ID_BARBARIAN_FURIOUS_CHARGE:			// 맹렬한 돌진
		return  new CBarbarian_Male_STF_Furious_Charge_Loop;
	...
	case SLOT_ID_BARBARIAN_EARTHQUAKE:				// 지진
		return  new CBarbarian_Male_STF_LeapAttack_End;
...
bool CBarbarian_Male_1HS_Mez::IsSlotRange(int _iIndex)
{
	//눌린 키 저장 
