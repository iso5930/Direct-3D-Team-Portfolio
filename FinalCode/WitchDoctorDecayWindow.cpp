...
CWitchDoctorDecayWindow::CWitchDoctorDecayWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_WitchDoctor_Decay"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("돌진 좀비"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("혼령 공세"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("산성 구름"));
	_tcscpy_s(m_tszName[3], MIN_STR, _T("죽음의 벽"));
	...
	_tcscpy_s(m_tszLevel[3], MIN_STR, _T("28"));
...
CWitchDoctorDecayWindow::~CWitchDoctorDecayWindow()
{
	Release();
}

void CWitchDoctorDecayWindow::Initialize()
{
	...
	// ZombieChargerButton(6)
	CSlotButton* pZombieChargerButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_DECAY, SLOT_ID_WITCHDOCTOR_ZOMBIE_CHARGER);
	m_pLayer->AddObject(pZombieChargerButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pZombieChargerButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pZombieChargerButton->Initialize();
	AddButton(pZombieChargerButton);

	// SpiritBarrageButton(7)
	CSlotButton* pSpiritBarrageButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_DECAY, SLOT_ID_WITCHDOCTOR_SPIRIT_BARRAGE);
	m_pLayer->AddObject(pSpiritBarrageButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pSpiritBarrageButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pSpiritBarrageButton->Initialize();
	AddButton(pSpiritBarrageButton);

	// AcidCloudButton(8)
	CSlotButton* pAcidCloudButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_DECAY, SLOT_ID_WITCHDOCTOR_ACID_CLOUD);
	m_pLayer->AddObject(pAcidCloudButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pAcidCloudButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pAcidCloudButton->Initialize();
	AddButton(pAcidCloudButton);

	// WallOfDeathButton(9)
	CSlotButton* pWallOfDeathButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_DECAY, SLOT_ID_WITCHDOCTOR_WALL_OF_DEATH);
	m_pLayer->AddObject(pWallOfDeathButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pWallOfDeathButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pWallOfDeathButton->Initialize();
	AddButton(pWallOfDeathButton);
...
int CWitchDoctorDecayWindow::Update()
{
	CWindowUI::Update();
	...
	if(m_ButtonVec[4]->IsLUp())
		CUIMgr::GetInstance()->NewWindow(WINDOW_TYPE_WITCHDOCTOR_VOODOO, m_iIndex, m_eSlotID);
...
void CWitchDoctorDecayWindow::Render()
{
	CWindowUI::Render();
...
void CWitchDoctorDecayWindow::Release()
{
	// DownArrowSign
...
void CWitchDoctorDecayWindow::Renew()
{
	// SelectedSkillButton
