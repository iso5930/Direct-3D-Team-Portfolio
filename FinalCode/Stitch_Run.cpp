...
CStitch_Run::CStitch_Run(int _iMode)
{
	// StateType
	m_eStateType = STATE_TYPE_STITCH_RUN;
...
CStitch_Run::~CStitch_Run(void)
{
}

void CStitch_Run::Initialize()
{
	...
	CSoundMgr::GetInstance()->PlaySoundForMonster(_T("Stitch_Run.ogg"));

	// Speed
	m_pPathFindCom->m_fSpeed = 200.0f;
	...
	if(D3DXVec3Length(&(pPlayerTransformCom->m_vPos - m_pTransformCom->m_vPos)) >= 120.0f)
		m_pPathFindCom->Enable();

}

CMonsterState* CStitch_Run::Action()
{
	switch(m_iMode)
	{
	case 0:
...
CMonsterState* CStitch_Run::Mode0()
{
	//전투
	// Player
	CObject* pPlayer = NULL;
	...
	if(D3DXVec3Length(&(pPlayerTransformCom->m_vPos - m_pTransformCom->m_vPos)) < 120.0f)
	{
		...
		{
		case 0:
			return new CStitch_Attack;
		case 1:
			return new CStitch_Attack2;
		case 2:
			return new CStitch_Attack3;
		}
	}
...
CMonsterState* CStitch_Run::Mode1()
{
	// 원거리 공격에 피격당했을 시
	// Player
	CObject* pPlayer = NULL;
