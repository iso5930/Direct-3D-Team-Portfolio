...
CWitchDoctorHauntSlot::CWitchDoctorHauntSlot()
{
	// SlotType
	m_eSlotType = SLOT_TYPE_WITCHDOCTOR_SECONDARY;

	// SlotID
	m_eSlotID = SLOT_ID_WITCHDOCTOR_HAUNT;
...
SLOT_RESULT CWitchDoctorHauntSlot::Begin(int _iMode)
{
	// Check Enable
	if (!m_bEnable)
	{
	...
	// Check Mana
	if (m_pOwner->GetStat()->iMana < 50 * (1.0f - m_pOwner->GetReducedManaConsumption()))
	{
	...
	// Mana
	m_pOwner->GetStat()->iMana -= int(50 * (1.0f - m_pOwner->GetReducedManaConsumption()));

	return SLOT_RESULT_NULL;
}

SLOT_RESULT CWitchDoctorHauntSlot::Action()
{
	// Check Pick
	D3DXVECTOR3 vPos;
	if(m_pOwner->GetPickObject() == NULL)
...
SLOT_RESULT CWitchDoctorHauntSlot::End()
{
	return SLOT_RESULT_NULL;
}
