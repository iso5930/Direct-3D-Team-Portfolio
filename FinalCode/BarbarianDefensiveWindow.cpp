...
CBarbarianDefensiveWindow::CBarbarianDefensiveWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_Barbarian_Defensive"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("발 구르기"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("도약 공격"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("질주"));
	_tcscpy_s(m_tszName[3], MIN_STR, _T("고통 감내"));
...
CBarbarianDefensiveWindow::~CBarbarianDefensiveWindow()
{
	Release();
}

void CBarbarianDefensiveWindow::Initialize()
{
	...
	// GroundStompButton(6)
	CSlotButton* pGroundStompButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_BARBARIAN_DEFENSIVE, SLOT_ID_BARBARIAN_GROUND_STOMP);
	m_pLayer->AddObject(pGroundStompButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pGroundStompButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pGroundStompButton->Initialize();
	AddButton(pGroundStompButton);

	// LeapButton(7)
	CSlotButton* pLeapButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_BARBARIAN_DEFENSIVE, SLOT_ID_BARBARIAN_LEAP);
	m_pLayer->AddObject(pLeapButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pLeapButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pLeapButton->Initialize();
	AddButton(pLeapButton);

	// SprintButton(8)
	CSlotButton* pSprintButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_BARBARIAN_DEFENSIVE, SLOT_ID_BARBARIAN_SPRINT);
	m_pLayer->AddObject(pSprintButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pSprintButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pSprintButton->Initialize();
	AddButton(pSprintButton);

	// IgnorePainButton(9)
	CSlotButton* pIgnorePainButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_BARBARIAN_DEFENSIVE, SLOT_ID_BARBARIAN_IGNORE_PAIN);
	m_pLayer->AddObject(pIgnorePainButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pIgnorePainButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pIgnorePainButton->Initialize();
	AddButton(pIgnorePainButton);
...
int CBarbarianDefensiveWindow::Update()
{
	CWindowUI::Update();
	...
	if(m_ButtonVec[4]->IsLUp())
		CUIMgr::GetInstance()->NewWindow(WINDOW_TYPE_BARBARIAN_MIGHT, m_iIndex, m_eSlotID);
...
void CBarbarianDefensiveWindow::Render()
{
	CWindowUI::Render();
...
void CBarbarianDefensiveWindow::Release()
{
	// DownArrowSign
...
void CBarbarianDefensiveWindow::Renew()
{
	// SelectedSkillButton
