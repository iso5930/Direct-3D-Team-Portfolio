...
CZombie_Skinny_Male_Red_Idle::CZombie_Skinny_Male_Red_Idle(int _iMode)
{
	// StateType
	m_eStateType = STATE_TYPE_ZOMBIE_SKINNY_MALE_RED_IDLE;
...
CZombie_Skinny_Male_Red_Idle::~CZombie_Skinny_Male_Red_Idle(void)
{
}

void CZombie_Skinny_Male_Red_Idle::Initialize()
{
...
CMonsterState* CZombie_Skinny_Male_Red_Idle::Action()
{
	switch(m_iMode)
	{
	case 0:
...
CMonsterState* CZombie_Skinny_Male_Red_Idle::Mode0()
{	
	// Player
		...
		if(m_pPathFindCom->IsMove(&vNextPos))
		{
			...
			if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
			{
				...
				if(Equals(fDotZ, 1.0f, MAX_EPSILON))
				{
					...
					return new CZombie_Skinny_Male_Red_Walk1(0);
				}
				else
				{
					if(fDotX > 0.0f)
...
CMonsterState* CZombie_Skinny_Male_Red_Idle::Mode1()
{
	// Player
	CObject* pPlayer = NULL;
	m_pOwner->GetLayer()->FindObject(pPlayer, _T("Player"), LAYER_TYPE_PLAYER);
...
CMonsterState* CZombie_Skinny_Male_Red_Idle::Mode2()
{
	...
	if(m_fWorryTime > m_fWorryMaxTime)
	{
		...
		case 0:
			m_fWorryTime = 0.0f;
			m_bWorry = true;
			return new CZombie_Skinny_Male_Red_Attack1;
		case 1:
			m_fWorryTime = 0.0f;
			m_bWorry = true;
			return new CZombie_Skinny_Male_Red_Attack2;
		case 2:
			m_fWorryTime = 0.0f;
			m_bWorry = true;
			return new CZombie_Skinny_Male_Red_Attack3;
		case 3:
			m_fWorryTime = 0.0f;
			m_bWorry = true;
			return new CZombie_Skinny_Male_Red_Attack4;
		}
	}
