...
CWizardPrimaryWindow::CWizardPrimaryWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_Wizard_Primary"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("¸¶·ÂÅº"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("Àü±â Ãæ°Ý"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("Àú½ÂÀÇ Ä®³¯"));
	_tcscpy_s(m_tszName[3], MIN_STR, _T("°¨Àü"));
...
CWizardPrimaryWindow::~CWizardPrimaryWindow()
{
	Release();
}

void CWizardPrimaryWindow::Initialize()
{
	...
	// MagicMissileButton(6)
	CSlotButton* pMagicMissileButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WIZARD_PRIMARY, SLOT_ID_WIZARD_MAGIC_MISSILE);
	m_pLayer->AddObject(pMagicMissileButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pMagicMissileButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pMagicMissileButton->Initialize();
	AddButton(pMagicMissileButton);

	// ShockPulseButton(7)
	CSlotButton* pShockPulseButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WIZARD_PRIMARY, SLOT_ID_WIZARD_SHOCK_PULSE);
	m_pLayer->AddObject(pShockPulseButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pShockPulseButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pShockPulseButton->Initialize();
	AddButton(pShockPulseButton);

	// SpectralBladeButton(8)
	CSlotButton* pSpectralBladeButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WIZARD_PRIMARY, SLOT_ID_WIZARD_SPECTRAL_BLADE);
	m_pLayer->AddObject(pSpectralBladeButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pSpectralBladeButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pSpectralBladeButton->Initialize();
	AddButton(pSpectralBladeButton);

	// ElectrocuteButton(9)
	CSlotButton* pElectrocuteButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WIZARD_PRIMARY, SLOT_ID_WIZARD_ELECTROCUTE);
	m_pLayer->AddObject(pElectrocuteButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pElectrocuteButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pElectrocuteButton->Initialize();
	AddButton(pElectrocuteButton);
...
int CWizardPrimaryWindow::Update()
{
	CWindowUI::Update();
...
void CWizardPrimaryWindow::Render()
{
	CWindowUI::Render();
...
void CWizardPrimaryWindow::Release()
{
	// DownArrowSign
...
void CWizardPrimaryWindow::Renew()
{
	// SelectedSkillButton
