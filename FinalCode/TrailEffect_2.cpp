...
CTrailEffect_2::CTrailEffect_2(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, DWORD _dwMaxCnt, D3DXMATRIX* _pMatrix, D3DXVECTOR3* _pUp, D3DXVECTOR3* _pDown, float _fPeriod, TCHAR* _tszTexKey)
...
CTrailEffect_2::~CTrailEffect_2(void)
{
	Release();
}

void CTrailEffect_2::Initialize()
{
	CObject::Initialize();
}

int CTrailEffect_2::Update()
{
	CObject::Update();
...
void CTrailEffect_2::Render()
{
	...
	{
		m_pVertex[i].vTexUV.x = 1.0f - (float(i / 2) / float(m_dwMaxCnt / 2 - 1));
...
void CTrailEffect_2::Release()
{
	delete[] m_pVertex;
}

void CTrailEffect_2::Add(D3DXVECTOR3* _pUp, D3DXVECTOR3* _pDown)
{	
