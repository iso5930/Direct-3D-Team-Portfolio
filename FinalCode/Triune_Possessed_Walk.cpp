...
CTriune_Possessed_Walk::CTriune_Possessed_Walk(int _iMode)
{
	// StateType
	m_eStateType = STATE_TYPE_TRIUNE_POSSESSED_WALK;
...
CTriune_Possessed_Walk::~CTriune_Possessed_Walk(void)
{
}

void CTriune_Possessed_Walk::Initialize()
{
...
CMonsterState* CTriune_Possessed_Walk::Action()
{
	switch(m_iMode)
	{
	case 0:
...
CMonsterState* CTriune_Possessed_Walk::Mode0()
{
	//비전투
...
CMonsterState* CTriune_Possessed_Walk::Mode1()
{
	//전투
	// Player
	CObject* pPlayer = NULL;
	...
	if(D3DXVec3Length(&(pPlayerTransformCom->m_vPos - m_pTransformCom->m_vPos)) > 200.0f)
		m_fAttack2Time += CTimeMgr::GetInstance()->GetDeltaTime();
	...
	if(m_fAttack2Time < 3.f)
	{
		if(D3DXVec3Length(&(pPlayerTransformCom->m_vPos - m_pTransformCom->m_vPos)) < 80.0f)
		{
			...
			return new CTriune_Possessed_Idle(1);
		}
	}
	else
	{
		...
		if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
		{
			...
			m_fAttack2Time = 0.0f;
			return new CTriune_Possessed_Attack2_Middle;
		}
	}
...
CMonsterState* CTriune_Possessed_Walk::Mode2()
{
	//전투
	// Player
	CObject* pPlayer = NULL;
