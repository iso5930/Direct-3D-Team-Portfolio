...
CBarbarianRageWindow::CBarbarianRageWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_Barbarian_Rage"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("지진"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("선조의 귀환"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("광전사의 진노"));
...
CBarbarianRageWindow::~CBarbarianRageWindow()
{
	Release();
}

void CBarbarianRageWindow::Initialize()
{
	...
	// EarthquakeButton(6)
	CSlotButton* pEarthquakeButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_BARBARIAN_RAGE, SLOT_ID_BARBARIAN_EARTHQUAKE);
	m_pLayer->AddObject(pEarthquakeButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pEarthquakeButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pEarthquakeButton->Initialize();
	AddButton(pEarthquakeButton);

	// CallOfTheAncientsButton(7)
	CSlotButton* pCallOfTheAncientsButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_BARBARIAN_RAGE, SLOT_ID_BARBARIAN_CALL_OF_THE_ANCIENTS);
	m_pLayer->AddObject(pCallOfTheAncientsButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pCallOfTheAncientsButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.0f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pCallOfTheAncientsButton->Initialize();
	AddButton(pCallOfTheAncientsButton);

	// WrathOfTheBerserkerButton(8)
	CSlotButton* pWrathOfTheBerserkerButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_BARBARIAN_RAGE, SLOT_ID_BARBARIAN_WRATH_OF_THE_BERSERKER);
	m_pLayer->AddObject(pWrathOfTheBerserkerButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pWrathOfTheBerserkerButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pWrathOfTheBerserkerButton->Initialize();
	AddButton(pWrathOfTheBerserkerButton);
...
int CBarbarianRageWindow::Update()
{
	CWindowUI::Update();
...
void CBarbarianRageWindow::Render()
{
	CWindowUI::Render();
...
void CBarbarianRageWindow::Release()
{
	// DownArrowSign
...
void CBarbarianRageWindow::Renew()
{
	// SelectedSkillButton
