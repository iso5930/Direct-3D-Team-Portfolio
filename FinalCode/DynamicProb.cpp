...
CDynamicProb::CDynamicProb(TCHAR* _tszObjKey, OBJ_TYPE _eObjType)
: CObject(_tszObjKey, _eObjType)
{
	// UpdateCom
	CUpdateCom* pUpdateCom = new CUpdateCom(UPDATE_TYPE_DYNAMIC_PROB);
	AddComponent(pUpdateCom);

	// RenderCom
	m_pRenderCom = new CRenderCom(RENDER_TYPE_DYNAMIC_PROB);
...
CDynamicProb::~CDynamicProb()
{
	Release();
}

void CDynamicProb::Initialize()
{
	CObject::Initialize();
...
int CDynamicProb::Update()
{
	...
	if(m_bIsOver)
	{
		// Over
		CMouse* pMouse = (CMouse*)pFindObject;
		pMouse->Over(MOUSE_OVER_TYPE_DYNAMIC_PROB, this);
...
void CDynamicProb::Event()
{
}

void CDynamicProb::EventEnd()
{
}

void CDynamicProb::DisOver()
{
	// IsOver
	m_bIsOver = false;
...
void CDynamicProb::Intersect(D3DXVECTOR3* _pSrc, CNaviCell* _pSrcCell, D3DXVECTOR3* _pDst, CNaviCell** _ppDstCell)
{
	// CollisionCom
	...
	*_ppDstCell = NULL;
	...
	for(int i = 0; i < 4; ++i)
	{
		...
		{
			if(line[i].Classify(_pSrc) != POINT_CLASS_RIGHT_SIDE)
			{
				// NextPos
...
void CDynamicProb::Intersect(D3DXVECTOR3* _pSrc, D3DXVECTOR3* _pDst)
{
	// CollisionCom
	CCollisionCom* pCollisionCom = (CCollisionCom*)m_pComponent[COM_TYPE_COLLISION];
	if(pCollisionCom == NULL)
