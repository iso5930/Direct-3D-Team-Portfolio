...
CTriune_Berserker_Idle::CTriune_Berserker_Idle(void)
{
	// StateType
	m_eStateType = STATE_TYPE_TRIUNE_BERSERKER_IDLE;
...
void CTriune_Berserker_Idle::Initialize()
{
	CMonsterState::Initialize();
	m_pOwner->SetAnimation(0);

	CSoundMgr::GetInstance()->PlaySoundForMonster(_T("Berserker_Idle.ogg"));
}

CMonsterState* CTriune_Berserker_Idle::Action()
{
	// Player
			...
			if(m_pPathFindCom->IsMove(&vNextPos))
			{
				...
				if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
				{
					...
					if(Equals(fDotZ, 1.0f, MAX_EPSILON))
					{
						...
						//Walk
						return new CTriune_Berserker_Run(0);
