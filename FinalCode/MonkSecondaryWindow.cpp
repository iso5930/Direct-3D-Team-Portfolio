...
CMonkSecondaryWindow::CMonkSecondaryWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_Monk_Secondary"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("¹Ý´ÞÂ÷±â"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("ÆøÇ³ ÁúÁÖ"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("ºûÀÇ ÆÄµ¿"));
...
CMonkSecondaryWindow::~CMonkSecondaryWindow()
{
	Release();
}

void CMonkSecondaryWindow::Initialize()
{
	...
	// LashingTailKickButton(6)
	CSlotButton* pLashingTailKickButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_SECONDARY, SLOT_ID_MONK_LASHING_TAIL_KICK);
	m_pLayer->AddObject(pLashingTailKickButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pLashingTailKickButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pLashingTailKickButton->Initialize();
	AddButton(pLashingTailKickButton);

	// TempestRushButton(7)
	CSlotButton* pTempestRushButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_SECONDARY, SLOT_ID_MONK_TEMPEST_RUSH);
	m_pLayer->AddObject(pTempestRushButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pTempestRushButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.0f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pTempestRushButton->Initialize();
	AddButton(pTempestRushButton);

	// WaveOfLightButton(8)
	CSlotButton* pWaveOfLightButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_SECONDARY, SLOT_ID_MONK_WAVE_OF_LIGHT);
	m_pLayer->AddObject(pWaveOfLightButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pWaveOfLightButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pWaveOfLightButton->Initialize();
	AddButton(pWaveOfLightButton);
...
int CMonkSecondaryWindow::Update()
{
	CWindowUI::Update();
...
void CMonkSecondaryWindow::Render()
{
	CWindowUI::Render();
...
void CMonkSecondaryWindow::Release()
{
	// DownArrowSign
...
void CMonkSecondaryWindow::Renew()
{
	// SelectedSkillButton
