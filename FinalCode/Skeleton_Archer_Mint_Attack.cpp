...
CSkeleton_Archer_Mint_Attack::CSkeleton_Archer_Mint_Attack(int _iMode)
{
	// StateType
	m_eStateType = STATE_TYPE_SKELETON_ARCHER_MINT_ATTACK;
...
CSkeleton_Archer_Mint_Attack::~CSkeleton_Archer_Mint_Attack(void)
{
}

void CSkeleton_Archer_Mint_Attack::Initialize()
{
...
CMonsterState* CSkeleton_Archer_Mint_Attack::Action()
{
...
CMonsterState* CSkeleton_Archer_Mint_Attack::Mode0()
{
	// Player
...
CMonsterState* CSkeleton_Archer_Mint_Attack::Mode1()
{
	// Player
	CObject* pPlayer = NULL;
	m_pOwner->GetLayer()->FindObject(pPlayer, _T("Player"), LAYER_TYPE_PLAYER);
	...
	if(D3DXVec3Length(&(pPlayerTransformCom->m_vPos - m_pTransformCom->m_vPos)) < 300.0f)
	{
		...
		if(m_pAnimController->m_dFrameTime - 0.25 <= m_pAnimController->GetTrackPos())
			return new CSkeleton_Archer_Mint_Attack(1);
	}
	else if(D3DXVec3Length(&(pPlayerTransformCom->m_vPos - m_pTransformCom->m_vPos)) > 300.0f)
	{
		if(m_pAnimController->m_dFrameTime - 0.25 <= m_pAnimController->GetTrackPos())
