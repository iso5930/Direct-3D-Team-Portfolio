...
CMonkDefensiveWindow::CMonkDefensiveWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_Monk_Defensive"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("´«ºÎ½Å ¼¶±¤"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("Ãµ»óÀÇ ¼û°á"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("Æò¾È"));
	_tcscpy_s(m_tszName[3], MIN_STR, _T("³»¸éÀÇ ¾È½ÄÃ³"));
...
CMonkDefensiveWindow::~CMonkDefensiveWindow()
{
	Release();
}

void CMonkDefensiveWindow::Initialize()
{
	...
	// BlindingFlashButton(6)
	CSlotButton* pBlindingFlashButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_DEFENSIVE, SLOT_ID_MONK_BLINDING_FLASH);
	m_pLayer->AddObject(pBlindingFlashButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pBlindingFlashButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pBlindingFlashButton->Initialize();
	AddButton(pBlindingFlashButton);

	// BreathOfHeavenButton(7)
	CSlotButton* pBreathOfHeavenButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_DEFENSIVE, SLOT_ID_MONK_BREATH_OF_HEAVEN);
	m_pLayer->AddObject(pBreathOfHeavenButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pBreathOfHeavenButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pBreathOfHeavenButton->Initialize();
	AddButton(pBreathOfHeavenButton);

	// SerenityButton(8)
	CSlotButton* pSerenityButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_DEFENSIVE, SLOT_ID_MONK_SERENITY);
	m_pLayer->AddObject(pSerenityButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pSerenityButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pSerenityButton->Initialize();
	AddButton(pSerenityButton);

	// InnerSanctuaryButton(9)
	CSlotButton* pInnerSanctuaryButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_MONK_DEFENSIVE, SLOT_ID_MONK_INNER_SANCTUARY);
	m_pLayer->AddObject(pInnerSanctuaryButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pInnerSanctuaryButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pInnerSanctuaryButton->Initialize();
	AddButton(pInnerSanctuaryButton);
...
int CMonkDefensiveWindow::Update()
{
	CWindowUI::Update();
	...
	if(m_ButtonVec[4]->IsLUp())
		CUIMgr::GetInstance()->NewWindow(WINDOW_TYPE_MONK_TECHNIQUES, m_iIndex, m_eSlotID);
...
void CMonkDefensiveWindow::Render()
{
	CWindowUI::Render();
...
void CMonkDefensiveWindow::Release()
{
	// DownArrowSign
...
void CMonkDefensiveWindow::Renew()
{
	// SelectedSkillButton
