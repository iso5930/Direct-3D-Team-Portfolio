...
CMonk_Male_DW_FF_Idle::CMonk_Male_DW_FF_Idle(void)
{
	m_eStateType = STATE_TYPE_MONK_MALE_DW_FF_IDLE;
...
void CMonk_Male_DW_FF_Idle::Initialize()
{
...
CPlayerState* CMonk_Male_DW_FF_Idle::Action()
{
	for (int i = 0 ; i < SKILL_TYPE_END ; ++i)
	{
		if (m_pOwner->IsMez(i))
		{
			return new CMonk_Male_DW_FF_Mez(i , D3DXVECTOR3(0.f , 0.f , 0.f ));
		}
	}
...
CPlayerState* CMonk_Male_DW_FF_Idle::Mode0()
{
...
CPlayerState* CMonk_Male_DW_FF_Idle::Mode1()
{
	...
	if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
	{
		...
		if(Equals(fDotZ, 1.0f, MAX_EPSILON))
		{
			...
			return new CMonk_Male_DW_FF_Run(m_pOwner->GetLastPushSlotNum());
		}
		else
		{
			if(fDotX > 0.0f)
...
CPlayerState* CMonk_Male_DW_FF_Idle::SlotAction(int _iIndex)
{
...
bool CMonk_Male_DW_FF_Idle::IsSlotRange(int _iIndex)
{
	//눌린 키 저장 
