...
CRect::CRect(CRect* _pRect)
{
	// BufType
	m_eBufType = BUF_TYPE_RECT;

	// RefCnt
	m_dwRefCnt = _pRect->m_dwRefCnt + 1;

	// FVF
	m_dwFVF = _pRect->m_dwFVF;

	// VtxSize
	m_dwVtxSize = _pRect->m_dwVtxSize;

	// VtxCnt
	m_dwVtxCnt = _pRect->m_dwVtxCnt;

	// IdxSize
	m_dwIdxSize = _pRect->m_dwIdxSize;

	// TriCnt
	m_dwTriCnt = _pRect->m_dwTriCnt;
