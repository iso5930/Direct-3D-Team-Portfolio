...
CSkeleton_Summoner_Red_Magic::CSkeleton_Summoner_Red_Magic(int _iMode)
{
	// StateType
	m_eStateType = STATE_TYPE_SKELETON_SUMMONER_RED_MAGIC;
...
CSkeleton_Summoner_Red_Magic::~CSkeleton_Summoner_Red_Magic(void)
{
}

void CSkeleton_Summoner_Red_Magic::Initialize()
{
...
CMonsterState* CSkeleton_Summoner_Red_Magic::Action()
{
	if(!m_bEffect && m_pAnimController->GetTrackPos() > 2.343f)
	{
		...
		D3DXVec3TransformCoord(&pTransformCom->m_vPos, &pTransformCom->m_vPos, &((CSkeleton_Summoner_Red*)m_pOwner)->m_LeftHandBone.mWorld);
...
CMonsterState* CSkeleton_Summoner_Red_Magic::Mode0()
{
	// Player
		...
		&& D3DXVec3Length(&(pPlayerTransformCom->m_vPos - m_pTransformCom->m_vPos)) > 200.0f)
	{
		...
		if(m_pAnimController->m_dFrameTime - 0.25 <= m_pAnimController->GetTrackPos())
			return new CSkeleton_Summoner_Red_Idle(1);
	...
	else if(D3DXVec3Length(&(pPlayerTransformCom->m_vPos - m_pTransformCom->m_vPos)) <= 200.0f)
	{
		if(m_pAnimController->m_dFrameTime - 0.25 <= m_pAnimController->GetTrackPos())
		{
			return new CSkeleton_Summoner_Red_Walk(2);
		}
	}
...
CMonsterState* CSkeleton_Summoner_Red_Magic::Mode1()
{
	// Player
	CObject* pPlayer = NULL;
	m_pOwner->GetLayer()->FindObject(pPlayer, _T("Player"), LAYER_TYPE_PLAYER);
