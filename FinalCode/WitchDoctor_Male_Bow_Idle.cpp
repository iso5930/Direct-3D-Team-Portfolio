...
CWitchDoctor_Male_Bow_Idle::CWitchDoctor_Male_Bow_Idle(void)
{
	m_eStateType = STATE_TYPE_WITCHDOCTOR_MALE_BOW_IDLE;
...
void CWitchDoctor_Male_Bow_Idle::Initialize()
{
...
CPlayerState* CWitchDoctor_Male_Bow_Idle::Action()
{
	for (int i = 0 ; i < SKILL_TYPE_END ; ++i)
	{
		if (m_pOwner->IsMez(i))
		{
			return new CWitchDoctor_Male_Bow_Mez(i , D3DXVECTOR3(0.f , 0.f , 0.f ));
		}
	}
...
CPlayerState* CWitchDoctor_Male_Bow_Idle::Mode0()
{
...
CPlayerState* CWitchDoctor_Male_Bow_Idle::Mode1()
{
	...
	if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
	{
		...
		if(Equals(fDotZ, 1.0f, MAX_EPSILON))
		{
			...
			return new CWitchDoctor_Male_Bow_Run(m_pOwner->GetLastPushSlotNum());
		}
		else
		{
			if(fDotX > 0.0f)
...
CPlayerState* CWitchDoctor_Male_Bow_Idle::SlotAction(int _iIndex)
{
...
bool CWitchDoctor_Male_Bow_Idle::IsSlotRange(int _iIndex)
{
	//눌린 키 저장 
