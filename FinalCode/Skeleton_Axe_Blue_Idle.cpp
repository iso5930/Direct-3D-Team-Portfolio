...
CSkeleton_Axe_Blue_Idle::CSkeleton_Axe_Blue_Idle(void)
{
	// StateType
	m_eStateType = STATE_TYPE_SKELETON_AXE_BLUE_IDLE;
...
void CSkeleton_Axe_Blue_Idle::Initialize()
{
	CMonsterState::Initialize();
	m_pOwner->SetAnimation(0);
}

CMonsterState* CSkeleton_Axe_Blue_Idle::Action()
{
	// Player
	...
	else //비전투
	{
		...
		if(m_fHoverTime > 5.94)
		{
			...
			{
				//임의의 좌표
				int iRand2 = rand() % 500 - 250;
				int iRand3 = rand() % 500 - 250;
			...
			if(m_pPathFindCom->IsMove(&vNextPos))
			{
				...
				if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
				{
					...
					if(Equals(fDotZ, 1.0f, MAX_EPSILON))
					{
						...
						//Walk
						return new CSkeleton_Axe_Blue_Run(0);
