...
CSkeleton_Summoner_Blue_Idle::CSkeleton_Summoner_Blue_Idle(int _iMode)
{
	// StateType
	m_eStateType = STATE_TYPE_SKELETON_SUMMONER_BLUE_IDLE;
...
CSkeleton_Summoner_Blue_Idle::~CSkeleton_Summoner_Blue_Idle(void)
{
}

void CSkeleton_Summoner_Blue_Idle::Initialize()
{
...
CMonsterState* CSkeleton_Summoner_Blue_Idle::Action()
{
	switch(m_iMode)
	{
	case 0:
...
CMonsterState* CSkeleton_Summoner_Blue_Idle::Mode0()
{
	// Player
			...
			if(m_pPathFindCom->IsMove(&vNextPos))
			{
				...
				if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
				{
					...
					if(Equals(fDotZ, 1.0f, MAX_EPSILON))
					{
						...
						//Walk
						return new CSkeleton_Summoner_Blue_Walk(0);
...
CMonsterState* CSkeleton_Summoner_Blue_Idle::Mode1()
{
