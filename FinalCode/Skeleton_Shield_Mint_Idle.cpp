...
CSkeleton_Shield_Mint_Idle::CSkeleton_Shield_Mint_Idle(void)
{
	// StateType
	m_eStateType = STATE_TYPE_SKELETON_SHIELD_MINT_IDLE;
...
void CSkeleton_Shield_Mint_Idle::Initialize()
{
	CMonsterState::Initialize();
	m_pOwner->SetAnimation(0);
}

CMonsterState* CSkeleton_Shield_Mint_Idle::Action()
{
	// Player
			...
			if(m_pPathFindCom->IsMove(&vNextPos))
			{
				...
				if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
				{
					...
					if(Equals(fDotZ, 1.0f, MAX_EPSILON))
					{
						...
						//Walk
						return new CSkeleton_Shield_Mint_Walk(0);
