...
CSkeleton_OneHand_Blue_Idle::CSkeleton_OneHand_Blue_Idle(void)
{
	// StateType
	m_eStateType = STATE_TYPE_SKELETON_ONEHAND_BLUE_IDLE;
...
void CSkeleton_OneHand_Blue_Idle::Initialize()
{
...
CMonsterState* CSkeleton_OneHand_Blue_Idle::Action()
{
	// Player
			...
			if(m_pPathFindCom->IsMove(&vNextPos))
			{
				...
				if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
				{
					...
					if(Equals(fDotZ, 1.0f, MAX_EPSILON))
					{
						...
						//Walk
						return new CSkeleton_OneHand_Blue_Run(0);
