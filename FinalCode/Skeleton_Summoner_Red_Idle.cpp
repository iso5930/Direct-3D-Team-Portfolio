...
CSkeleton_Summoner_Red_Idle::CSkeleton_Summoner_Red_Idle(int _iMode)
{
	// StateType
	m_eStateType = STATE_TYPE_SKELETON_SUMMONER_RED_IDLE;
...
CSkeleton_Summoner_Red_Idle::~CSkeleton_Summoner_Red_Idle(void)
{
}

void CSkeleton_Summoner_Red_Idle::Initialize()
{
...
CMonsterState* CSkeleton_Summoner_Red_Idle::Action()
{
	switch(m_iMode)
	{
	case 0:
...
CMonsterState* CSkeleton_Summoner_Red_Idle::Mode0()
{
	// Player
			...
			if(m_pPathFindCom->IsMove(&vNextPos))
			{
				...
				if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
				{
					...
					if(Equals(fDotZ, 1.0f, MAX_EPSILON))
					{
						...
						//Walk
						return new CSkeleton_Summoner_Red_Walk(0);
...
CMonsterState* CSkeleton_Summoner_Red_Idle::Mode1()
{
