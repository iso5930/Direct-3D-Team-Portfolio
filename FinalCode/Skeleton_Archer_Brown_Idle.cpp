...
CSkeleton_Archer_Brown_Idle::CSkeleton_Archer_Brown_Idle(int _iMode)
{
	// StateType
	m_eStateType = STATE_TYPE_SKELETON_ARCHER_BROWN_IDLE;
...
CSkeleton_Archer_Brown_Idle::~CSkeleton_Archer_Brown_Idle(void)
{
}

void CSkeleton_Archer_Brown_Idle::Initialize()
{
	CMonsterState::Initialize();
	m_pOwner->SetAnimation(0);
}

CMonsterState* CSkeleton_Archer_Brown_Idle::Action()
{
	switch(m_iMode)
	{
	case 0:
...
CMonsterState* CSkeleton_Archer_Brown_Idle::Mode0()
{
	// Player
		...
		if(m_pPathFindCom->IsMove(&vNextPos))
		{
			...
			if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
			{
				...
				if(Equals(fDotZ, 1.0f, MAX_EPSILON))
				{
					...
					//공격
					return new CSkeleton_Archer_Brown_Run(0);
				}
				else
				{
					if(fDotX > 0.0f)
			...
			if(m_pPathFindCom->IsMove(&vNextPos))
			{
				...
				if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
				{
					...
					if(Equals(fDotZ, 1.0f, MAX_EPSILON))
					{
						...
						//Walk
						return new CSkeleton_Archer_Brown_Walk;
...
CMonsterState* CSkeleton_Archer_Brown_Idle::Mode1()
{
	m_fWorryTime += CTimeMgr::GetInstance()->GetDeltaTime();

	if(m_fWorryTime > 1.f)
		return new CSkeleton_Archer_Brown_Attack(0);
	
	return NULL;
}

CMonsterState* CSkeleton_Archer_Brown_Idle::Mode2()
{
	m_fWorryTime += CTimeMgr::GetInstance()->GetDeltaTime();
