...
CDemonHunterPrimaryWindow::CDemonHunterPrimaryWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_DemonHunter_Primary"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("굶주린 화살"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("속박탄"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("올가미"));
	_tcscpy_s(m_tszName[3], MIN_STR, _T("회피 사격"));
	_tcscpy_s(m_tszName[4], MIN_STR, _T("수류탄"));
	...
	_tcscpy_s(m_tszLevel[3], MIN_STR, _T("14"));
	_tcscpy_s(m_tszLevel[4], MIN_STR, _T("15"));
...
CDemonHunterPrimaryWindow::~CDemonHunterPrimaryWindow()
{
	Release();
}

void CDemonHunterPrimaryWindow::Initialize()
{
	...
	// HungeringArrowButton(6)
	CSlotButton* pHungeringArrowButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_PRIMARY, SLOT_ID_DEMONHUNTER_HUNGERING_ARROW);
	m_pLayer->AddObject(pHungeringArrowButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pHungeringArrowButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.3f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pHungeringArrowButton->Initialize();
	AddButton(pHungeringArrowButton);

	// EntanglingShotButton(7)
	CSlotButton* pEntanglingShotButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_PRIMARY, SLOT_ID_DEMONHUNTER_ENTANGLING_SHOT);
	m_pLayer->AddObject(pEntanglingShotButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pEntanglingShotButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pEntanglingShotButton->Initialize();
	AddButton(pEntanglingShotButton);

	// BolaButton(8)
	CSlotButton* pBolaButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_PRIMARY, SLOT_ID_DEMONHUNTER_BOLA);
	m_pLayer->AddObject(pBolaButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pBolaButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.0f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pBolaButton->Initialize();
	AddButton(pBolaButton);

	// EvasiveFireButton(9)
	CSlotButton* pEvasiveFireButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_PRIMARY, SLOT_ID_DEMONHUNTER_EVASIVE_FIRE);
	m_pLayer->AddObject(pEvasiveFireButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pEvasiveFireButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pEvasiveFireButton->Initialize();
	AddButton(pEvasiveFireButton);

	// GrenadesButton(10)
	CSlotButton* pGrenadesButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_PRIMARY, SLOT_ID_DEMONHUNTER_GRENADES);
	m_pLayer->AddObject(pGrenadesButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pGrenadesButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.3f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pGrenadesButton->Initialize();
	AddButton(pGrenadesButton);
...
int CDemonHunterPrimaryWindow::Update()
{
	CWindowUI::Update();
	...
	if(m_ButtonVec[4]->IsLUp())
		CUIMgr::GetInstance()->NewWindow(WINDOW_TYPE_DEMONHUNTER_SECONDARY, m_iIndex, m_eSlotID);
...
void CDemonHunterPrimaryWindow::Render()
{
	CWindowUI::Render();
...
void CDemonHunterPrimaryWindow::Release()
{
	// DownArrowSign
...
void CDemonHunterPrimaryWindow::Renew()
{
	// SelectedSkillButton
