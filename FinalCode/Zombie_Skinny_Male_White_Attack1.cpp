...
CZombie_Skinny_Male_White_Attack1::CZombie_Skinny_Male_White_Attack1(void)
{
	// StateType
	m_eStateType = STATE_TYPE_ZOMBIE_SKINNY_MALE_WHITE_ATTACK1;
...
void CZombie_Skinny_Male_White_Attack1::Initialize()
{
	...
	CSoundMgr::GetInstance()->PlaySoundForMonster(_T("Zombie_Skinny_Male_Attack.ogg"));
}

CMonsterState* CZombie_Skinny_Male_White_Attack1::Action()
{
	m_fEffectCreateTime += CTimeMgr::GetInstance()->GetDeltaTime();

	if(m_fEffectCreateTime > 0.561f && !m_bEffect)
	{
		CZombie_Skinny_Male_Scratch_A* pScratchA = new CZombie_Skinny_Male_Scratch_A(NULL, OBJ_TYPE_DYNAMIC, m_pOwner, &(m_pTransformCom->m_vPos));
...
CMonsterState* CZombie_Skinny_Male_White_Attack1::Mode0()
{
...
CMonsterState* CZombie_Skinny_Male_White_Attack1::Mode1()
{
	m_fTime += CTimeMgr::GetInstance()->GetDeltaTime();
	if(m_fTime > 1.0f)
	{
