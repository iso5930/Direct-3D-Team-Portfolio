...
CTriune_Vessel_Idle::CTriune_Vessel_Idle(void)
{
	// StateType
	m_eStateType = STATE_TYPE_TRIUNE_VESSEL_IDLE;
...
void CTriune_Vessel_Idle::Initialize()
{
...
CMonsterState* CTriune_Vessel_Idle::Action()
{
	// Player
		...
		if(m_pPathFindCom->IsMove(&vNextPos))
		{
			...
			if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
			{
				...
				if(Equals(fDotZ, 1.0f, MAX_EPSILON))
				{
					...
					return new CTriune_Vessel_Walk;
				}
				else
				{
					if(fDotX > 0.0f)
