...
CBarbarian_Female::CBarbarian_Female(TCHAR* _tszObjKey, OBJ_TYPE _eObjType)
...
CBarbarian_Female::~CBarbarian_Female()
{
	Release();
}

void CBarbarian_Female::Initialize()
{
	CPlayer::Initialize();
}

int CBarbarian_Female::Update()
{
	CPlayer::Update();
...
void CBarbarian_Female::Render()
{
	CPlayer::Render();
}

void CBarbarian_Female::Release()
{
}

int CBarbarian_Female::Attack()
{
	// Attack
	CPlayer::Attack();
...
void CBarbarian_Female::LevelUp()
{
	// Stat
	m_stat.iStrength += 3;
	m_stat.iAgility += 1;
...
void CBarbarian_Female::Damage(CMonster* _pMonster)
{
	// Damage
	CPlayer::Damage(_pMonster);
}

void CBarbarian_Female::Damage(CEffect* _pEffect)
{
	// Damage
	CPlayer::Damage(_pEffect);
}

void CBarbarian_Female::Renew()
{
	// Renew
	CPlayer::Renew();
}

void CBarbarian_Female::CreateAttack()
{
	// CreateAttack
	CPlayer::CreateAttack();
}

void CBarbarian_Female::SetSlot(int _iIndex, CSlot* _pSlot)
{
	for(int i = 0; i < 7; ++i)
	{
		if(m_pSlot[i] != NULL)
...
int CBarbarian_Female::TotalStat1()
{
	// Strength
	int iStrength = GetStrength();
...
int CBarbarian_Female::TotalStat2()
{
	// Defense
	int iDefense = GetDef();
...
int CBarbarian_Female::TotalStat3()
{
	// LifeRecovery
