...
CZombie_Male_Walk::CZombie_Male_Walk(int _iMode)
{
	// StateType
	m_eStateType = STATE_TYPE_ZOMBIE_MALE_WALK;
...
CZombie_Male_Walk::~CZombie_Male_Walk(void)
{
}

void CZombie_Male_Walk::Initialize()
{
...
CMonsterState* CZombie_Male_Walk::Action()
{
	switch(m_iMode)
	{
	case 0:
...
CMonsterState* CZombie_Male_Walk::Mode0()
{
...
CMonsterState* CZombie_Male_Walk::Mode1()
{
	//전투
		...
		m_fWaitTime > 2.f)
	{
		...
		{
			m_fWaitTime = 0.0f;
			return new CZombie_Male_Idle(1);
		}
	}
	...
	if(D3DXVec3Length(&(pPlayerTransformCom->m_vPos - m_pTransformCom->m_vPos)) < 55.0f)
	{
		...
		return new CZombie_Male_Idle(2);
	}
...
CMonsterState* CZombie_Male_Walk::Mode2()
{
...
CMonsterState* CZombie_Male_Walk::Mode3()
{
	//전투
	m_fWaitTime += CTimeMgr::GetInstance()->GetDeltaTime();
