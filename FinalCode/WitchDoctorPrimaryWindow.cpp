...
CWitchDoctorPrimaryWindow::CWitchDoctorPrimaryWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_WitchDoctor_Primary"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("µ¶Ä§"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("½ÃÃ¼ °Å¹Ì"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("¿ªº´ µÎ²¨ºñ"));
	_tcscpy_s(m_tszName[3], MIN_STR, _T("ºÒÆøÅº"));
...
CWitchDoctorPrimaryWindow::~CWitchDoctorPrimaryWindow()
{
	Release();
}

void CWitchDoctorPrimaryWindow::Initialize()
{
	...
	// PoisonDartButton(6)
	CSlotButton* pPoisonDartButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_PRIMARY, SLOT_ID_WITCHDOCTOR_POISON_DART);
	m_pLayer->AddObject(pPoisonDartButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pPoisonDartButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pPoisonDartButton->Initialize();
	AddButton(pPoisonDartButton);

	// CorpseSpidersButton(7)
	CSlotButton* pCorpseSpidersButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_PRIMARY, SLOT_ID_WITCHDOCTOR_CORPSE_SPIDERS);
	m_pLayer->AddObject(pCorpseSpidersButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pCorpseSpidersButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pCorpseSpidersButton->Initialize();
	AddButton(pCorpseSpidersButton);

	// PlagueOfToadsButton(8)
	CSlotButton* pPlagueOfToadsButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_PRIMARY, SLOT_ID_WITCHDOCTOR_PLAGUE_OF_TOADS);
	m_pLayer->AddObject(pPlagueOfToadsButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pPlagueOfToadsButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pPlagueOfToadsButton->Initialize();
	AddButton(pPlagueOfToadsButton);

	// FirebombButton(9)
	CSlotButton* pFirebombButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_PRIMARY, SLOT_ID_WITCHDOCTOR_FIREBOMB);
	m_pLayer->AddObject(pFirebombButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pFirebombButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pFirebombButton->Initialize();
	AddButton(pFirebombButton);
...
int CWitchDoctorPrimaryWindow::Update()
{
	CWindowUI::Update();
...
void CWitchDoctorPrimaryWindow::Render()
{
	CWindowUI::Render();
...
void CWitchDoctorPrimaryWindow::Release()
{
	// DownArrowSign
...
void CWitchDoctorPrimaryWindow::Renew()
{
	// SelectedSkillButton
