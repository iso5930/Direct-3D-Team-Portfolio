...
CDemonHunterSecondaryWindow::CDemonHunterSecondaryWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_DemonHunter_Secondary"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("투검"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("연발 사격"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("회전 표창"));
	_tcscpy_s(m_tszName[3], MIN_STR, _T("원소 화살"));
...
CDemonHunterSecondaryWindow::~CDemonHunterSecondaryWindow()
{
	Release();
}

void CDemonHunterSecondaryWindow::Initialize()
{
	...
	// ImpaleButton(6)
	CSlotButton* pImpaleButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_SECONDARY, SLOT_ID_DEMONHUNTER_IMPALE);
	m_pLayer->AddObject(pImpaleButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pImpaleButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pImpaleButton->Initialize();
	AddButton(pImpaleButton);

	// RapidFireButton(7)
	CSlotButton* pRapidFireButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_SECONDARY, SLOT_ID_DEMONHUNTER_RAPID_FIRE);
	m_pLayer->AddObject(pRapidFireButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pRapidFireButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pRapidFireButton->Initialize();
	AddButton(pRapidFireButton);

	// ChakramButton(8)
	CSlotButton* pChakramButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_SECONDARY, SLOT_ID_DEMONHUNTER_CHAKRAM);
	m_pLayer->AddObject(pChakramButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pChakramButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pChakramButton->Initialize();
	AddButton(pChakramButton);

	// ElementalArrowButton(9)
	CSlotButton* pElementalArrowButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_SECONDARY, SLOT_ID_DEMONHUNTER_ELEMENTAL_ARROW);
	m_pLayer->AddObject(pElementalArrowButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pElementalArrowButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pElementalArrowButton->Initialize();
	AddButton(pElementalArrowButton);
...
int CDemonHunterSecondaryWindow::Update()
{
	CWindowUI::Update();
...
void CDemonHunterSecondaryWindow::Render()
{
	CWindowUI::Render();
...
void CDemonHunterSecondaryWindow::Release()
{
	// DownArrowSign
...
void CDemonHunterSecondaryWindow::Renew()
{
	// SelectedSkillButton
