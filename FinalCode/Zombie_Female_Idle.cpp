...
CZombie_Female_Idle::CZombie_Female_Idle(int _iMode)
{
	// StateType
	m_eStateType = STATE_TYPE_ZOMBIE_FEMALE_IDLE;
...
CZombie_Female_Idle::~CZombie_Female_Idle()
{
}

void CZombie_Female_Idle::Initialize()
{
	...
	CSoundMgr::GetInstance()->PlaySoundForMonster(_T("Zombie_Female_Idle.ogg"));
}

CMonsterState* CZombie_Female_Idle::Action()
{
	switch(m_iMode)
	{
	case 0:
...
CMonsterState* CZombie_Female_Idle::Mode0()
{
	// Player
			...
			if(m_pPathFindCom->IsMove(&vNextPos))
			{
				...
				if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
				{
					...
					if(Equals(fDotZ, 1.0f, MAX_EPSILON))
					{
						...
						//Walk
						return new CZombie_Female_Walk(0);
...
CMonsterState* CZombie_Female_Idle::Mode1()
{
...
CMonsterState* CZombie_Female_Idle::Mode2()
{
	...
	if(m_fWorryTime > m_fWorryMaxTime)
	{
		...
		case 0:
			m_fWorryTime = 0.0f;
			m_bWorry = true;
			return new CZombie_Female_Attack;
		case 1:
			m_fWorryTime = 0.0f;
			m_bWorry = true;
			return new CZombie_Female_Attack2;
		case 2:
			m_fWorryTime = 0.0f;
			m_bWorry = true;
			return new CZombie_Female_Walk(2);
		}
	}
