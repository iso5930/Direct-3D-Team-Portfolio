...
CDemonHunterDefensiveWindow::CDemonHunterDefensiveWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_DemonHunter_Defensive"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("¼è¸ø µ£"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("¿¬¸·"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("¾îµÒÀÇ Èû"));
...
CDemonHunterDefensiveWindow::~CDemonHunterDefensiveWindow()
{
	Release();
}

void CDemonHunterDefensiveWindow::Initialize()
{
	...
	// CaltropsButton(6)
	CSlotButton* pCaltropsButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_DEFENSIVE, SLOT_ID_DEMONHUNTER_CALTROPS);
	m_pLayer->AddObject(pCaltropsButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pCaltropsButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pCaltropsButton->Initialize();
	AddButton(pCaltropsButton);

	// SmokeScreenButton(7)
	CSlotButton* pSmokeScreenButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_DEFENSIVE, SLOT_ID_DEMONHUNTER_SMOKE_SCREEN);
	m_pLayer->AddObject(pSmokeScreenButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pSmokeScreenButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.0f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pSmokeScreenButton->Initialize();
	AddButton(pSmokeScreenButton);

	// ShadowPowerButton(8)
	CSlotButton* pShadowPowerButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_DEMONHUNTER_DEFENSIVE, SLOT_ID_DEMONHUNTER_SHADOW_POWER);
	m_pLayer->AddObject(pShadowPowerButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pShadowPowerButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pShadowPowerButton->Initialize();
	AddButton(pShadowPowerButton);
...
int CDemonHunterDefensiveWindow::Update()
{
	CWindowUI::Update();
...
void CDemonHunterDefensiveWindow::Render()
{
	CWindowUI::Render();
...
void CDemonHunterDefensiveWindow::Release()
{
	// DownArrowSign
...
void CDemonHunterDefensiveWindow::Renew()
{
	// SelectedSkillButton
