...
CSkeleton_OneHand_Blue::CSkeleton_OneHand_Blue(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iLevel)
: CMonster(_tszObjKey, _eObjType, _iLevel)
{
	// Info
	_tcscpy_s(m_info.tszName, MIN_STR, _T("되돌아온 자"));

	// Stat
	m_stat.iLife = m_stat.iMaxLife = _iLevel * 32000;
	m_stat.iStrength = _iLevel * 60;
	...
	pBufferCom->AddBuffer(_T("Skeleton_OneHand_Blue"));
...
CSkeleton_OneHand_Blue::~CSkeleton_OneHand_Blue()
{
	Release();
}

void CSkeleton_OneHand_Blue::Initialize()
{
	CMonster::Initialize();
}

int CSkeleton_OneHand_Blue::Update()
{
	CMonster::Update();
...
void CSkeleton_OneHand_Blue::Render()
{
	CMonster::Render();
}

void CSkeleton_OneHand_Blue::Release()
{
}

void CSkeleton_OneHand_Blue::CreateAttack()
{
	...
	{
	case STATE_TYPE_SKELETON_ONEHAND_BLUE_ATTACK :
		...
		break;
	case STATE_TYPE_SKELETON_ONEHAND_BLUE_DEATH :
		break;
	case STATE_TYPE_SKELETON_ONEHAND_BLUE_HIT :
		break;
	case STATE_TYPE_SKELETON_ONEHAND_BLUE_IDLE :
		break;
	case STATE_TYPE_SKELETON_ONEHAND_BLUE_RUN :
		break;
	case STATE_TYPE_SKELETON_ONEHAND_BLUE_SPAWN :
		break;
	}
...
void CSkeleton_OneHand_Blue::Damage(CPlayer* _pPlayer)
{
	// PreLife
	...
	{
		SetState(new CSkeleton_OneHand_Blue_Death);
		return;
	}
	...
	{
		SetState(new CSkeleton_OneHand_Blue_Hit);
		return;
	}
	else
	{
		if(m_pState != NULL && m_pState->GetStateType() != STATE_TYPE_SKELETON_ONEHAND_BLUE_RUN)
		{
			SetState(new CSkeleton_OneHand_Blue_Run(2));
			return;
		}
	}
}

void CSkeleton_OneHand_Blue::Damage(CEffect* _pEffect)
{
	// PreLife
	int iPreLife = m_stat.iLife;
