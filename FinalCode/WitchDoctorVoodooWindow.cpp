...
CWitchDoctorVoodooWindow::CWitchDoctorVoodooWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_WitchDoctor_Voodoo"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("덩치"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("대재앙의 부두술"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("우상족 군대"));
...
CWitchDoctorVoodooWindow::~CWitchDoctorVoodooWindow()
{
	Release();
}

void CWitchDoctorVoodooWindow::Initialize()
{
	...
	// GargantuanButton(6)
	CSlotButton* pGargantuanButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_VOODOO, SLOT_ID_WITCHDOCTOR_GARGANTUAN);
	m_pLayer->AddObject(pGargantuanButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pGargantuanButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pGargantuanButton->Initialize();
	AddButton(pGargantuanButton);

	// BigBadVoodooButton(7)
	CSlotButton* pBigBadVoodooButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_VOODOO, SLOT_ID_WITCHDOCTOR_BIG_BAD_VOODOO);
	m_pLayer->AddObject(pBigBadVoodooButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pBigBadVoodooButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.0f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pBigBadVoodooButton->Initialize();
	AddButton(pBigBadVoodooButton);

	// FetishArmyButton(8)
	CSlotButton* pFetishArmyButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_VOODOO, SLOT_ID_WITCHDOCTOR_FETISH_ARMY);
	m_pLayer->AddObject(pFetishArmyButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pFetishArmyButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pFetishArmyButton->Initialize();
	AddButton(pFetishArmyButton);
...
int CWitchDoctorVoodooWindow::Update()
{
	CWindowUI::Update();
...
void CWitchDoctorVoodooWindow::Render()
{
	CWindowUI::Render();
...
void CWitchDoctorVoodooWindow::Release()
{
	// DownArrowSign
...
void CWitchDoctorVoodooWindow::Renew()
{
	// SelectedSkillButton
