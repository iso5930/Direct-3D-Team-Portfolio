...
CWitchDoctorSecondaryWindow::CWitchDoctorSecondaryWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_WitchDoctor_Secondary"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("¸ÁÀÚÀÇ ¼Õ¾Æ±Í"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("ºÒ¹ÚÁã"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("È¥·É Ãâ¸ô"));
	_tcscpy_s(m_tszName[3], MIN_STR, _T("¸Þ¶Ñ±â ¶¼"));
...
CWitchDoctorSecondaryWindow::~CWitchDoctorSecondaryWindow()
{
	Release();
}

void CWitchDoctorSecondaryWindow::Initialize()
{
	...
	// GraspOfTheDeadButton(6)
	CSlotButton* pGraspOfTheDeadButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_SECONDARY, SLOT_ID_WITCHDOCTOR_GRASP_OF_THE_DEAD);
	m_pLayer->AddObject(pGraspOfTheDeadButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pGraspOfTheDeadButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pGraspOfTheDeadButton->Initialize();
	AddButton(pGraspOfTheDeadButton);

	// FirebatsButton(7)
	CSlotButton* pFirebatsButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_SECONDARY, SLOT_ID_WITCHDOCTOR_FIREBATS);
	m_pLayer->AddObject(pFirebatsButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pFirebatsButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pFirebatsButton->Initialize();
	AddButton(pFirebatsButton);

	// HauntButton(8)
	CSlotButton* pHauntButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_SECONDARY, SLOT_ID_WITCHDOCTOR_HAUNT);
	m_pLayer->AddObject(pHauntButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pHauntButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pHauntButton->Initialize();
	AddButton(pHauntButton);

	// LocustSwarmButton(9)
	CSlotButton* pLocustSwarmButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WITCHDOCTOR_SECONDARY, SLOT_ID_WITCHDOCTOR_LOCUST_SWARM);
	m_pLayer->AddObject(pLocustSwarmButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pLocustSwarmButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pLocustSwarmButton->Initialize();
	AddButton(pLocustSwarmButton);
...
int CWitchDoctorSecondaryWindow::Update()
{
	CWindowUI::Update();
	...
	if(m_ButtonVec[3]->IsLUp())
		CUIMgr::GetInstance()->NewWindow(WINDOW_TYPE_WITCHDOCTOR_PRIMARY, m_iIndex, m_eSlotID);
...
void CWitchDoctorSecondaryWindow::Render()
{
	CWindowUI::Render();
...
void CWitchDoctorSecondaryWindow::Release()
{
	// DownArrowSign
...
void CWitchDoctorSecondaryWindow::Renew()
{
	// SelectedSkillButton
