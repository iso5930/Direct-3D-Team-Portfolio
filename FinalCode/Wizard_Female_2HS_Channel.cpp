...
CWizard_Female_2HS_Channel::CWizard_Female_2HS_Channel(int _iPushSlot)
{
	m_eStateType = STATE_TYPE_WIZARD_FEMALE_2HS_CHANNEL;
...
CWizard_Female_2HS_Channel::~CWizard_Female_2HS_Channel(void)
{
...
void CWizard_Female_2HS_Channel::Initialize()
{
...
CPlayerState* CWizard_Female_2HS_Channel::Action()
{
...
CPlayerState* CWizard_Female_2HS_Channel::Mode0()
{
	...
	if (IsSlotRange(iSlotNum))
		...
		return new CWizard_Female_2HS_Run(m_pOwner->GetLastPushSlotNum());
};

CPlayerState* CWizard_Female_2HS_Channel::Mode1()
{
	// Find Priority Key and SlotNo
	int iSlotNum = CPlayerState::UsingKeyMouse();
...
CPlayerState* CWizard_Female_2HS_Channel::SlotAction(int _iIndex)
{
...
bool CWizard_Female_2HS_Channel::IsSlotRange(int _iIndex)
{
	//눌린 키 저장 
