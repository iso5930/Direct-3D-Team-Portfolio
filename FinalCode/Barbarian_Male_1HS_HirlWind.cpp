...
CBarbarian_Male_1HS_HirlWind::CBarbarian_Male_1HS_HirlWind(int _iPushSlot)
{
	m_eStateType = STATE_TYPE_BARBARIAN_MALE_1HS_HIRLWIND;
...
CBarbarian_Male_1HS_HirlWind::~CBarbarian_Male_1HS_HirlWind(void)
{
	m_pOwner->GetSlot(m_iPushSlot)->End();

}

void CBarbarian_Male_1HS_HirlWind::Initialize()
{
...
CPlayerState* CBarbarian_Male_1HS_HirlWind::Action()
{
...
CPlayerState* CBarbarian_Male_1HS_HirlWind::Mode0()
{
	if(m_pAnimController->m_dFrameTime <= m_pAnimController->GetTrackPos())
	{
		return new CBarbarian_Male_1HS_HirlWind(m_iPushSlot);
	}

	return NULL;
}

CPlayerState* CBarbarian_Male_1HS_HirlWind::Mode1()
{
	return new CBarbarian_Male_1HS_Idle;	
}

CPlayerState* CBarbarian_Male_1HS_HirlWind::SlotAction(int _iIndex)
{
	...
	case SLOT_ID_BARBARIAN_BASH:					// 후려치기
		{
			int iRand = rand() % 2; 
			if( 0 == iRand )
				return new CBarbarian_Male_1HS_Attack;
			else
				return new CBarbarian_Male_1HS_Cleave02;
		}
	case SLOT_ID_BARBARIAN_CLEAVE:					// 가르기
		{
			int iRand = rand() % 2; 
			if( 0 == iRand )
				return new CBarbarian_Male_1HS_Cleave01;
			else
				return new CBarbarian_Male_1HS_Cleave02;
		}
	...
	case SLOT_ID_BARBARIAN_HAMMER_OF_THE_ANCIENTS:	// 선조의 망치
		return new CBarbarian_Male_1HS_Revenge;
	...
	case SLOT_ID_BARBARIAN_WHIRLWIND:				// 소용돌이
		return new CBarbarian_Male_1HS_HirlWind(_iIndex);

	case SLOT_ID_BARBARIAN_ANCIENT_SPEAR:			// 고대의 작살
		return new CBarbarian_Male_1HS_Attack;

	case SLOT_ID_BARBARIAN_GROUND_STOMP:			// 발 구르기
		return  new CBarbarian_Male_1HS_FootStomp;

	case SLOT_ID_BARBARIAN_LEAP:					// 도약 공격
		return  new CBarbarian_Male_1HS_LeapAttack_Air;

	case SLOT_ID_BARBARIAN_SPRINT:					// 질주
		return new CBarbarian_Male_1HS_Buff;
	...
	case SLOT_ID_BARBARIAN_FURIOUS_CHARGE:			// 맹렬한 돌진
		return  new CBarbarian_Male_1HS_Furious_Charge_Loop;
