...
CBarbarianGroundStompSlot::CBarbarianGroundStompSlot()
{
	// SlotType
	m_eSlotType = SLOT_TYPE_BARBARIAN_DEFENSIVE;

	// SlotID
	m_eSlotID = SLOT_ID_BARBARIAN_GROUND_STOMP;
...
SLOT_RESULT CBarbarianGroundStompSlot::Begin(int _iMode)
{
	// Check Enable
	if (!m_bEnable)
	{
	...
	// Check Rage
	if(m_pOwner->GetStat()->iRage < int(15 * (1.0f - m_pOwner->GetReducedRageConsumption())))
	{
...
SLOT_RESULT CBarbarianGroundStompSlot::Action()
{
	return SLOT_RESULT_NULL;
}

SLOT_RESULT CBarbarianGroundStompSlot::End()
{
	return SLOT_RESULT_NULL;
}
