...
CWizard_FarAttack_ArrowEffect::CWizard_FarAttack_ArrowEffect(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, CObject* _pOwner, D3DXVECTOR3* _pDir)
...
CWizard_FarAttack_ArrowEffect::~CWizard_FarAttack_ArrowEffect(void)
{
	Release();
}

void CWizard_FarAttack_ArrowEffect::Initialize()
{
	CEffect::Initialize();

	m_pTrailEffect = new CWizard_FarAttack_TrailEffect(NULL, OBJ_TYPE_DYNAMIC, &m_pTransformCom->m_mWorld);
	m_pLayer->AddObject(m_pTrailEffect, LAYER_TYPE_EFFECT);
	m_pTrailEffect->Initialize();
}

int CWizard_FarAttack_ArrowEffect::Update()
{
	CEffect::Update();
...
void CWizard_FarAttack_ArrowEffect::Render()
{
	CEffect::Render();
}

void CWizard_FarAttack_ArrowEffect::Release()
{
	// TrailEffect
