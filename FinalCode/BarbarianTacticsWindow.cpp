...
CBarbarianTacticsWindow::CBarbarianTacticsWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_Barbarian_Tactics"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("위협의 외침"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("전투 격노"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("전장의 함성"));
	...
	_tcscpy_s(m_tszLevel[2], MIN_STR, _T("28"));
...
CBarbarianTacticsWindow::~CBarbarianTacticsWindow()
{
	Release();
}

void CBarbarianTacticsWindow::Initialize()
{
	...
	// ThreateningShoutButton(6)
	CSlotButton* pThreateningShoutButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_BARBARIAN_TACTICS, SLOT_ID_BARBARIAN_THREATENING_SHOUT);
	m_pLayer->AddObject(pThreateningShoutButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pThreateningShoutButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pThreateningShoutButton->Initialize();
	AddButton(pThreateningShoutButton);

	// BattleRageButton(7)
	CSlotButton* pBattleRageButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_BARBARIAN_TACTICS, SLOT_ID_BARBARIAN_BATTLE_RAGE);
	m_pLayer->AddObject(pBattleRageButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pBattleRageButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.0f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pBattleRageButton->Initialize();
	AddButton(pBattleRageButton);

	// WarCryButton(8)
	CSlotButton* pWarCryButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_BARBARIAN_TACTICS, SLOT_ID_BARBARIAN_WAR_CRY);
	m_pLayer->AddObject(pWarCryButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pWarCryButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.15f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pWarCryButton->Initialize();
	AddButton(pWarCryButton);
...
int CBarbarianTacticsWindow::Update()
{
	CWindowUI::Update();
...
void CBarbarianTacticsWindow::Render()
{
	CWindowUI::Render();
...
void CBarbarianTacticsWindow::Release()
{
	// DownArrowSign
...
void CBarbarianTacticsWindow::Renew()
{
	// SelectedSkillButton
