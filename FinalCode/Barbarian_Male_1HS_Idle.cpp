...
CBarbarian_Male_1HS_Idle::CBarbarian_Male_1HS_Idle(void)
{
	m_eStateType = STATE_TYPE_BARBARIAN_MALE_1HS_IDLE;
...
void CBarbarian_Male_1HS_Idle::Initialize()
{
...
CPlayerState* CBarbarian_Male_1HS_Idle::Action()
{
...
CPlayerState* CBarbarian_Male_1HS_Idle::Mode0()
{
...
CPlayerState* CBarbarian_Male_1HS_Idle::Mode1()
{
	...
	if(!Equals(vDir.x, 0.0f, MIN_EPSILON) || !Equals(vDir.z, 0.0f, MIN_EPSILON))
	{
		...
		if(Equals(fDotZ, 1.0f, MAX_EPSILON))
		{
			...
			return new CBarbarian_Male_1HS_Run(m_pOwner->GetLastPushSlotNum());
		}
		else
		{
			if(fDotX > 0.0f)
...
CPlayerState* CBarbarian_Male_1HS_Idle::SlotAction(int _iIndex)
{
...
bool CBarbarian_Male_1HS_Idle::IsSlotRange(int _iIndex)
{
	//눌린 키 저장 
