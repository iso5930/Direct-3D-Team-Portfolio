...
CTriune_Possessed_Attack2_End::CTriune_Possessed_Attack2_End(void)
{
	// StateType
	m_eStateType = STATE_TYPE_TRIUNE_POSSESSED_ATTACK2_END;
...
void CTriune_Possessed_Attack2_End::Initialize()
{
	...
	CSoundMgr::GetInstance()->PlaySoundForMonster(_T("Possessed_Attack2.ogg"));
}

CMonsterState* CTriune_Possessed_Attack2_End::Action()
{	
	m_fEffectCreateTime += CTimeMgr::GetInstance()->GetDeltaTime();

	if(m_fEffectCreateTime > 0.132f && !m_bEffect)
	{
		CTriune_Possessed_Scratch_B* pScratchB = new CTriune_Possessed_Scratch_B(NULL, OBJ_TYPE_DYNAMIC, m_pOwner, &(m_pTransformCom->m_vPos));
...
CMonsterState* CTriune_Possessed_Attack2_End::Mode0()
{
...
CMonsterState* CTriune_Possessed_Attack2_End::Mode1()
{	
	m_fTime += CTimeMgr::GetInstance()->GetDeltaTime();
	if(m_fTime > 1.0f)
	{
