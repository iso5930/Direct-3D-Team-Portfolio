...
CWizardSecondaryWindow::CWizardSecondaryWindow(TCHAR* _tszObjKey, OBJ_TYPE _eObjType, int _iIndex, SLOT_ID _eSlotID)
: CWindowUI(_tszObjKey, _eObjType, _T("Window_Wizard_Secondary"))
{
	// Select
	m_iSelect = -1;
	...
	_tcscpy_s(m_tszName[0], MIN_STR, _T("서리 광선"));
	_tcscpy_s(m_tszName[1], MIN_STR, _T("비전 보주"));
	_tcscpy_s(m_tszName[2], MIN_STR, _T("비전 격류"));
	_tcscpy_s(m_tszName[3], MIN_STR, _T("파열"));
...
CWizardSecondaryWindow::~CWizardSecondaryWindow()
{
	Release();
}

void CWizardSecondaryWindow::Initialize()
{
	...
	// RayOfFrostButton(6)
	CSlotButton* pRayOfFrostButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WIZARD_SECONDARY, SLOT_ID_WIZARD_RAY_OF_FROST);
	m_pLayer->AddObject(pRayOfFrostButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pRayOfFrostButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pRayOfFrostButton->Initialize();
	AddButton(pRayOfFrostButton);

	// ArcaneOrbButton(7)
	CSlotButton* pArcaneOrbButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WIZARD_SECONDARY, SLOT_ID_WIZARD_ARCANE_ORB);
	m_pLayer->AddObject(pArcaneOrbButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pArcaneOrbButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(-0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pArcaneOrbButton->Initialize();
	AddButton(pArcaneOrbButton);

	// ArcaneTorrentButton(8)
	CSlotButton* pArcaneTorrentButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WIZARD_SECONDARY, SLOT_ID_WIZARD_ARCANE_TORRENT);
	m_pLayer->AddObject(pArcaneTorrentButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pArcaneTorrentButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.075f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pArcaneTorrentButton->Initialize();
	AddButton(pArcaneTorrentButton);

	// DisintegrateButton(9)
	CSlotButton* pDisintegrateButton = new CSlotButton(NULL, OBJ_TYPE_STATIC, SLOT_TYPE_WIZARD_SECONDARY, SLOT_ID_WIZARD_DISINTEGRATE);
	m_pLayer->AddObject(pDisintegrateButton, LAYER_TYPE_UI);
	pTransformCom = (CTransformCom*)pDisintegrateButton->GetComponent(COM_TYPE_TRANSFORM);
	pTransformCom->m_vPos = D3DXVECTOR3(0.225f, 0.508f, 0.0f);
	pTransformCom->m_vSize = D3DXVECTOR3(0.078f, 0.159f, 0.0f);
	pDisintegrateButton->Initialize();
	AddButton(pDisintegrateButton);
...
int CWizardSecondaryWindow::Update()
{
	CWindowUI::Update();
...
void CWizardSecondaryWindow::Render()
{
	CWindowUI::Render();
...
void CWizardSecondaryWindow::Release()
{
	// DownArrowSign
...
void CWizardSecondaryWindow::Renew()
{
	// SelectedSkillButton
